...
v3: fixed back stepping - excluding the edge neigboring the previous triangle    
...
plt.figure()
kd=0
for lonp0 in np.arange(-70.,-65.,0.5):
    for latp0 in np.arange(39.,43.,0.5):
        lonp=lonp0*1.
        latp=latp0*1.
        kd=kd+1
        # find the triangle to which the current drifter position belongs
        print 'kd,lonp,latp',kd,lonp,latp
        try:
            kf,lamb0,lamb1,lamb2=find_kf_lonlat2(Grid,lonp,latp)
            kfp=kf*1 
            t=0. 
            print 't',t
            plt.plot(lonp,latp,'bo')
