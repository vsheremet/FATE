...
#subsample every 8th node
gom3r_llon,gom3r_llat=np.meshgrid(gom3r_lon[::8],gom3r_lat[::8])
RSHAPE=gom3r_llon.shape
"""

DATADIR='DriftTrackData/start0212/'
FTS='19800212'
FVS='0'
...
# plot drifter positions every 7 day
for kt in range(0,NT,7):
    plt.figure();
    plt.plot(lon,lat,'g.',lonc,latc,'c+',lont[kt,:],latt[kt,:],'r.');
...
for kd in range(0,ND,MD):
    plt.plot(lont[0,kd],latt[0,kd],'yo',lont[:,kd],latt[:,kd],'m-',lont[-1,kd],latt[-1,kd],'mo')
#plt.plot(llondx,llatdx,'yo');
"""


print 'start inconvexpolygon t0'
GB150m=np.load('GB150m.npy')
lont0=lont[0,:];latt0=latt[0,:]
i=inconvexpolygon(lont0,latt0,GB150m[:,0],GB150m[:,1])
print 'finish inconvexpolygon t0'
kdt0=np.argwhere(i).flatten() # drifterd that were initially in GB150m Area

lontGB=lont[:,kdt0]
lattGB=latt[:,kdt0]
NDGB=len(kdt0)

## plot every 10th drifter
#for kd in range(0,NDGB,MD):
#    plt.plot(lontGB[0,kd],lattGB[0,kd],'bo',lontGB[:,kd],lattGB[:,kd],'r-',lontGB[-1,kd],lattGB[-1,kd],'ro')

print 'start inconvexpolygon all t'
lontGBshape=lontGB.shape
iGB=inconvexpolygon(lontGB.flatten(),lattGB.flatten(),GB150m[:,0],GB150m[:,1])
print 'finish inconvexpolygon all t'
fGB=iGB*1.0
fGB=fGB.reshape(lontGBshape)
nGB=fGB.sum(1)
